...
class ProcessMGMT:
    ...
    def start_vnc(self, vnc_port):
        if vnc_port in self.vnc_process_list:
            ...
            time.sleep(0.05)
            chromium_process = subprocess.Popen(['chromium', '--profile-directory={}'.format(vnc_port), '--user-data-dir={}/instances/{}'.format(env['HOME'], vnc_port), '--kiosk', 'https://chess.com'], env=env)
